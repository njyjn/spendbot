...
export default withApiAuthRequired(async function handler(
  ...
  try {
    ...
    const lastMonth = moment(month, "MMM YY", true)
      .subtract(1, "months")
      .format("MMM YY");
    ...
    try {
      ...
      if (thisMonthSheetData && thisMonthSheetData[0].properties?.sheetId) {
        return res
          .status(400)
          .json({ error: "Sheet for requested month already exists" });
      }
    } catch (e: any) {}

    const lastMonthSheetData = (
      await service.spreadsheets.get({
        spreadsheetId: SHEET_ID,
        ranges: [`${lastMonth}!A1:A2`],
        includeGridData: false,
      })
    ).data.sheets;
    if (lastMonthSheetData) {
      ...
      if (
        duplicateResult.data.replies &&
        duplicateResult.data.replies[0].duplicateSheet?.properties?.sheetId
      ) {
